<template>
  <div class="modal-overlay" @click="$emit('close')">
    <div class="modal-content modal-lg" @click.stop>
      ...
      <form v-if="!loading" @submit.prevent="handleSubmit" class="modal-body">
        <div class="form-row">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const loadProduct = async () => {
      try {
        const product = await printifyStore.getProduct(props.productId)
        form.value = {
          ...product,
          images: product.images.map(url => ({ url, isExisting: true }))
        }
      } catch (error) {
        uiStore.notifyError('Failed to load product')
        emit('close')
      } finally {
        loading.value = false
      }
    ...
    const handleSubmit = async () => {
      try {
        ...
        // Append images
        const newImages = form.value.images.filter(img => !img.isExisting)
        ...
        await printifyStore.updateProduct(props.productId, formData)
        uiStore.notifySuccess('Product updated successfully')
        emit('close')
      } catch (error) {
        uiStore.notifyError('Failed to update product')
      } finally {
        saving.value = false
      }
    }
